...
InternationalTransferRouter.post("/submit", checkAuth, async (req, res) => {
  try {
    const {
      withdrawMethod, amount, pin, otp_code, Description, balance_type,
      // Wire Transfer
      accountName, accountNumber, bankName, bankaddress, Accounttype, country,
      iban, swiftCode, routingNumber,
      // Crypto
      cryptoCurrency, cryptoNetwork, wallet_address,
      // PayPal
      paypalEmail,
      // Wise
      wiseFullName, wiseEmail, wiseCountry,
      // Skrill
      skrillEmail, skrillFullName,
      // Venmo
      venmoUsername, venmoPhone,
      // Zelle
      zelleEmail, zellePhone, zelleName,
      // Cash App
      cashAppTag, cashAppFullName,
      // Revolut
      revolutFullName, revolutEmail, revolutPhone,
      // Alipay
      alipayId, alipayFullName,
      // WeChat Pay
      wechatId, wechatName,
    } = req.body;
    ...
    if (!withdrawMethod)
      return res.json({ success: false, message: "Transfer method is required." });
    ...
    if (!pin || String(pin).length < 4)
      return res.json({ success: false, message: "Transaction PIN is required." });
    ...
    if (!user.isAllowedToTransfer) {
      return res.json({
        success: false,
        message: user.blockedTransferReason || "Transfers are currently disabled on your account. Please contact support.",
      });
    }

    const { valid, reason } = verifyOTP(req.user._id, otp_code, "international_transfer");
    if (!valid) return res.json({ success: false, message: reason });

    if (!user.pin)
      return res.json({ success: false, message: "No transaction PIN set. Please set one in settings." });

    if (String(pin) !== String(user.pin))
      return res.json({ success: false, message: "Incorrect transaction PIN." });
    ...
    switch (withdrawMethod) {
      case "Wire Transfer":
        Object.assign(transferDetails, {
          accountName, accountNumber, bankName, bankAddress: bankaddress,
          accountType: Accounttype, country, iban, swiftCode, routingNumber,
        });
        break;
      case "Cryptocurrency":
        Object.assign(transferDetails, { cryptoCurrency, cryptoNetwork, walletAddress: wallet_address });
        break;
      case "PayPal":
        Object.assign(transferDetails, { paypalEmail });
        break;
      case "Wise Transfer":
        Object.assign(transferDetails, { fullName: wiseFullName, email: wiseEmail, country: wiseCountry });
        break;
      case "Skrill":
        Object.assign(transferDetails, { email: skrillEmail, fullName: skrillFullName });
        break;
      case "Venmo":
        Object.assign(transferDetails, { username: venmoUsername, phone: venmoPhone });
        break;
      case "Zelle":
        Object.assign(transferDetails, { email: zelleEmail, phone: zellePhone, fullName: zelleName });
        break;
      case "Cash App":
        Object.assign(transferDetails, { cashtag: cashAppTag, fullName: cashAppFullName });
        break;
      case "Revolut":
        Object.assign(transferDetails, { fullName: revolutFullName, email: revolutEmail, phone: revolutPhone });
        break;
      case "Alipay":
        Object.assign(transferDetails, { alipayId, fullName: alipayFullName });
        break;
    ...
    if (isCrypto) {
      await UserModel.findByIdAndUpdate(user._id, { $inc: { crypto_balance: -parsedAmount } });
    } else {
      await UserModel.findByIdAndUpdate(user._id, { $inc: { balance: -parsedAmount } });
    }
...
InternationalTransferRouter.get("/", checkAuth, async (req, res) => {
  try {
    ...
    const filter = { userId, type: "international" };
